...
import { transferOwnership } from "@/lib/services/membership-service";
...
/**
 * POST /api/groups/[groupId]/transfer - Transfer ownership (owner only)
 */
  ...
  try {
    ...
    if (!newOwnerUserId || typeof newOwnerUserId !== "string") {
      return NextResponse.json(
        { error: "Validation failed", details: { newOwnerUserId: ["Required"] } },
        { status: 400 }
      );
    }

    await transferOwnership(groupId, userId, newOwnerUserId);
